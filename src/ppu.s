...
.render_sprite:
    ; b has the x offset, c has the y offset and a has the sprite offset
...
.loop_x:
    ld a, b
    cp 199 ; bounds check. check 199 instead of 200 so that we know we can add two pixels to the screen
    jp nc, .next_y
    inc b
...
.next_y:
    ...
    ld a, c
    cp 150
    jp nc, .y_out_of_bounds
    ; restore to iy and add 200 to the pixel map addr
    pop iy
    ld e, 200
    ld d, 0
    add iy, de
...
; Each sprite entry is 3 bytes large so each lookup table entry needs to be 3 bytes after the previous one
coord_lookup: ; 573
